...
`just agent-feedback "<note>"` writes a single JSON line to `logs/agent-friction.jsonl` capturing the moment a workflow bonked. The corpus accumulates over weeks. Periodic review surfaces recurring patterns worth encoding into AGENTS.md, a new SKILL.md, or a new tool.
...
- As a venting channel about AGENTS.md being inconvenient or about not liking a constraint.
...
Recurring entries (2+ separate sessions on the same theme) are the strongest signal that the pattern deserves a fix — a AGENTS.md note, a new SKILL.md, or a tool change.

## Hook safety

`logs/agent-friction.jsonl` is at the root of `logs/` and is **not** under the `logs/tuned-*` or `logs/baseline-*` prefixes that `.Codex/hooks/no-log-overwrite.py` protects. Append-writes are allowed without prompting.
