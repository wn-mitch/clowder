...
- Drift > ±10% on a characteristic metric is observed and per AGENTS.md needs a hypothesis to ship.
...
- **`just sweep`** — single-sweep primitive. `hypothesize` calls it twice. Use `sweep` directly only when you have no hypothesis (rare; AGENTS.md prefers hypothesis-driven sweeps).
