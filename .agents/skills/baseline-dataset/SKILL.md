...
`logs/baseline-<LABEL>/` is protected by `.Codex/hooks/no-log-overwrite.py`. The orchestrator refuses to overwrite an existing baseline directory in the canonical path — rename or delete the old one first if you intend to rebuild under the same label.
