...
- ✓ landed **106** (done, ai-substrate, score 0.91 (cross-cluster)) — HungerUrgency modifier — substrate axis for Starvation interrupt retirement
- ✓ landed **164** (done, ai-substrate, score 0.91 (cross-cluster)) — Seed-42 (38,22) kitten cohort starves despite KittenCryMap
- ✓ landed **209** (done, balance, score 0.90 (cross-cluster)) — Positive colony_food_security axis on higher-tier DSEs
...
- 2026-05-03: Curve-shape fix + leading-input pair landed (this commit). The Iter 1 scaffolding shipped `(1 − hunger)^k` which is non-zero across the entire hunger range; integrated over a 1.34M-tick soak it killed even well-fed cats (Mocha focal-trace: hunger min=0.50, never below 0.15, "starved" at tick 1.21M from cumulative drain — see `docs/balance/starvation-rebalance.md` Iter 4). Fix: damage curve is now threshold-gated — `cliff_factor = 0` for `hunger ≥ starvation_cliff_threshold` (default `0.15`, mirroring the Maslow ladder's `critical_hunger_interrupt_threshold`); below threshold, ramps as `((threshold − hunger) / threshold)^k`. Brief mid-hunger excursions take **zero** damage. Paired `hunger_urgency_curve_exponent` knob added to `ScoringConstants` (default `1.0` linear = current behavior; treatment `0.4` sub-linear = nerve-impulse leading shape, lift saturates by hunger=0.15 at 83%). New field on `NeedsConstants`: `starvation_cliff_threshold`. Verified by paired single-seed soaks (per `feedback_chain_rare_events.md` — Starvation is at-floor in baseline, sweeps can't compute % from 0, structural verification beats sweep gating): default and treatment both produce 8 deaths total / 0 Starvation / same predator+wildlife distribution. The earlier Iter 3 8/8-Starvation wipeout was a stale-release-binary artifact (binary built at 14:54, code edits at 18:25-18:29; sweep ran the old code while `applied_overrides` cosmetically echoed the patch — serde silently dropped unknown fields at deserialization). 1806 unit tests pass. **Next:** courtship-canary regression in current main blocks welfare-axis sweeps for items 2/5; promotion of `starvation_cliff_use_legacy: false` to ship-default deferred until upstream regressions clear. Status stays `in-progress`.
- 2026-05-05: **Item 4 (hunting-success audit) closes affirmatively** — ticket 149 ("hunt-success disambiguation") landed instrumentation that disambiguates per-Hunt-action rate (25.6% on Iter 1) from per-discrete-attempt rate. Post-149 seed-42 soak measures **31.78% per-discrete-attempt success (504 kills / 1586 attempts), inside the 30–50% real-cat-biology band**. Iter 1's headline 25.6% was per-Hunt-action; it conflated within-Hunt retargeting with discrete attempts. The Iter 1 estimate of 34.4% (corrected) was off by ~2.6pp because it assumed every kill triggered "seeking another target" replan; 8/504 kills were consumed-on-spot instead. Per-species variance is enormous (mouse 84.9%, fish 6.9%) — two follow-on tickets recommended in `docs/balance/starvation-rebalance.md` Iter 6: unreachable-prey targeting bug, bird teleport-flee narrative gap. **Item 4 of §Scope closes; items 1–3, 5 remain in-progress as previously logged.**
- 2026-05-10: **Item 3 (breeding-floor lower 0.6 → 0.4)** satisfied by ticket 272 against post-257 substrate. Verified seed-42 soak-trace Mocha 900s (commit `3444d2d9`-dirty): `never_fired_expected_positives = []` (was `[MatingOccurred]`); `kittens_born = 2` (was 0); `continuity.courtship` 1609 → 2251 (+40%); `bonds_formed` 21 → 26; `positive_features_active` 34 → 38. Direction matches §Scope item 3's prediction (`courtship ↑ 50–200%` band; observed +40% on top of 257's already-elevated baseline). **Surfaces 187 (kitten starvation under active mating pipeline) as the next bottleneck**: Dawnkit-28 starves tick 1321484 — same pattern 187 documented from the post-184 reproduction. Items 1, 2, 5 of §Scope remain in-progress.
- 2026-05-18: Landing summary across 5 scope items: Item 1 (graded starvation cliff) substrate + curve fix shipped 2026-05-03 Iter 5 (threshold-gated ramp at src/systems/needs.rs:109-134, paired hunger_urgency_curve_exponent at src/ai/modifier.rs), treatment sweep at logs/sweep-threshold-gated-graded-cliff-drain-only-below-hunger-0-15-pa-treatment passing per user confirmation. Item 2 (per-life-stage multipliers, kitten 2.0× / young 1.3× / elder 1.5×) substrate shipped; treatment sweep at logs/sweep-per-life-stage-starvation-multipliers-kitten-2-0-young-1-3-a-treatment. Item 3 (breeding-hunger floor 0.6→0.4) landed via ticket 272. Item 4 (hunting-success audit) closed affirmatively via ticket 149 — per-discrete-attempt rate 31.78%, inside 30–50% real-cat-biology band; was a measurement artifact, no tuning needed. Item 5 (body-condition welfare axis) substrate shipped behind use_body_condition_for_breeding_gate flag; sweep concordant at +43.2% courtship_tally vs baseline (docs/balance/routing-the-mating-gate-through-the-body-condition-welfare-a.md). Downstream 187 kitten-starvation cascade resolved by ticket 398 (RaiseOffspringAspiration); current-main seed-42 footer: Starvation==0, 3 kittens weaned. Default-flip from legacy to graded cliff + body-condition gate deferred to follow-on ticket 416; the substrate engineering is complete.
