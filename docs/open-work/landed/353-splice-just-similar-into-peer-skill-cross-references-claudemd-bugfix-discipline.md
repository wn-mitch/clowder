...
- ✓ landed **354** (done, tooling-diagnostics-ui, score 0.83) — Parallel-session orchestration: /work skill + three-track partition + refinery
- ✓ landed ** 14** (done, —, score 0.83 (cross-cluster)) — "§4.2 State marker trio — `InCombat` / `OnCorruptedTile` / `OnSpecialTerrain` a…
