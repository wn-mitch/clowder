...
## Ticket 002 — Hunt-approach pipeline failures: code landed, measurement deferred (2026-04-26)

**Landed:** four code commits on bookmark `wnmitch/002-hunt-approach`
on top of `yvmxxtmv` (post-027 Bug 3). **Measurement-soak validation
deferred** — Hunt is not currently load-bearing on the canonical
seed-42 deep-soak (farming + foraging fully cover food production
post-027), so the per-lever soak attribution the ticket called for
produces zero signal. Code stays in for the future state where Hunt
becomes load-bearing again.

**Commit A — pre-init footer counters** (`snorskol`). Pre-initialize
`"EngagePrey: lost prey during approach"` and `"EngagePrey: stuck
while stalking"` to 0 in `EventLog::default()`, mirroring the
`continuity_tallies` pattern. Without pre-init, JSON diffs treat
"key absent" and "value 0" identically; pre-init makes the footer
carry these two counters on every soak. New unit test asserts both
keys are present in `Default`.

**Commit B.1 — stale-`dist` fix in EngagePrey APPROACH arm**
(`vskrynsp`). `dist` was captured at `resolve_engage_prey` entry
(line 4262) and reused unchanged in the give-up check at line 4471
after the cat moved up to `approach_speed` tiles toward the prey.
Pre-movement distance bled into a post-movement decision: cats whose
this-tick movement actually closed inside `approach_give_up_distance`
still bailed with "lost prey during approach". Fix: shadow `dist`
with a fresh `pos.manhattan_distance(&prey_pos)` immediately before
the give-up condition. Bug is APPROACH-only — STALK (line 4453) and
CHASE (line 4418) give-ups read only `state.no_move_ticks`. Worth
landing independent of measurement: it's a real lifted-condition bug
that over-fires the approach-failure mode whenever Hunt does run.

**Commit B.2 — stochastic fractional `stalk_speed`** (`wulkkour`).
Extract the implicit 1-tile/tick stalk closing rate at goap.rs:4432
into a tunable `DispositionConstants.stalk_speed: f32`, defaulting
to **1.25**. The STALK arm now loops `floor(stalk_speed)` guaranteed
`step_toward` calls plus one extra step rolled with probability
`stalk_speed - floor(stalk_speed)`. Gives finer per-tick tuning than
the integer `chase_speed` / `approach_speed` loops. Move-tracking
and `no_move_ticks` accounting unchanged.

**Commit C — per-prey detection-roll cadence gate** (`ntuqxylr`).
`try_detect_cat` was rolling a Bernoulli detection check on every
prey-AI tick. Add `PreyState.detection_cooldown_ticks: u8` (default
0) and `PreyConstants.detection_cadence_ticks: u8` (default 3). A
new `consume_detection_cooldown` helper gates the roll: positive
cooldown decrements and skips, zero cooldown rolls and resets to
`cadence - 1`. Stagger is per-prey (no synchronous detection waves),
reset is unconditional once the gate opens. Two unit tests cover the
gate in isolation. Adjusted one existing integration test
(`prey_alert_detects_nearby_cat`) to set cadence=1 locally so its
detection-path assertion remains stable independent of cadence.

**Why measurement deferred.** Soak finding (seed-42, 900s release,
post-Bug-3 trunk in clowder-002 isolated workspace): 0 Hunt plans,
0 PreyKilled across 38,600 ticks. Cats correctly choose Crafting /
Resting / Exploring over Hunt because food stockpile climbs from
0 → 32/50 via crop-tend (13,775) and forage (13,960 plans), and
`food_available: true` throughout. Hunt scores 0.46–0.64 for hungry
cats, Herbcraft scores 0.70–0.94 — and that's the design ("Hunt is
fine to lose when they have food, that's the whole point"). The
ticket's original 1,981-Hunt-plans diagnostic described an older
sim state where Hunt was load-bearing; current canonical scenario
no longer exercises Hunt meaningfully.

**Re-soak when relevant.** Whenever a future scenario, seed, or
balance change brings Hunt back into the canonical load-bearing
position, rebase `wnmitch/002-hunt-approach` onto trunk and run the
four-soak sequence (Commit-A baseline → post-B.1 → post-B.2 → C
isolated → combined) per the original ticket plan. The pre-init
counters make the footer comparison clean from the first run.

**Files touched:** `src/resources/event_log.rs` (Commit A pre-init +
test); `src/systems/goap.rs` (B.1 stale-dist fix; B.2 stochastic
loop); `src/resources/sim_constants.rs` (B.2 `stalk_speed`; C
`detection_cadence_ticks`); `src/components/prey.rs` (C
`detection_cooldown_ticks`); `src/systems/prey.rs` (C cooldown gate
+ helper + tests). 1,331 tests pass.

## PE-001 — Test harness drift resolved (2026-04-21, `a869ef1`)

Three failing integration tests (`cats_eat_when_hungry`,
`simulation_is_deterministic`, `simulation_runs_1000_ticks_without_panic`)
restored to green. Drift cause: `tests/integration.rs::setup_world` had
fallen behind `src/plugins/setup.rs::build_new_world` (missing four
resources and four cat components); commit `a869ef1` adds the missing
inserts. The structural cause — two parallel world-construction sites —
was retired four days later by ticket 030 (`b9129a1`). Detail page kept
under `docs/open-work/pre-existing/test-harness-drift.md`.

## Ticket 027 — Mating cadence three-bug cascade (2026-04-25 → 2026-04-26)

**Landed:** Bug 1 2026-04-25 (`218db85c`) | Bug 2 2026-04-25 (`367a05e4`) | Bug 3 2026-04-26 (this commit). **Ticket:** 027 (Mating cadence — three-bug cascade blocking MatingOccurred).

The `baseline-2026-04-25` dataset showed `MatingOccurred = 0` across all
15 sweep runs. Diagnosis pinned three distinct structural bugs at three
different layers of the §7.M three-layer Mating model. None were
coefficient-tunable — each needed a code fix before any balance-tuning
iteration on mating cadence would become meaningful.

**Bug 1 — observability decoupling.** `Feature::CourtshipInteraction`
was emitted only inside the `MateWith` step resolver, so the `courtship`
continuity canary measured "MateWith executed with a target" not "any
courtship-related interaction". Since MateWith never ran (Bug 2's
deadlock), the canary read zero. **Fix:** emit
`Feature::CourtshipInteraction` and push a new
`EventKind::CourtshipDrifted` event inside the courtship-drift gate in
`social.rs::check_bonds`. `Feature::CourtshipInteraction` was promoted
out of the rare-legend exempt list. Acceptance
(`continuity_tallies.courtship > 0`) cleared by 840×.

**Bug 2 — lifted-condition outer gate.** `MateDse` was ported in Phase 4
with its own `EligibilityFilter` but the legacy outer wrapper at
`src/ai/scoring.rs:916` (`if ctx.has_eligible_mate { ... }`) was never
retired — same lifted-condition pattern CLAUDE.md flags from the
2026-04-23 §7.2 regression. **Fix:** retire the wrapper. Promote
`mating::update_mate_eligibility_markers` from a stub to a real
per-tick author of the `HasEligibleMate` ZST. `MateDse.eligibility()`
carries `.require(HasEligibleMate::KEY)`. `mate_eligibility_q` snapshot
in `evaluate_and_plan` / `evaluate_dispositions` populates the marker
table. Acceptance (`mate` L2 records > 0 in every focal trace where
the cat is Adult / not-Incapacitated) cleared with 34,949 records.

**Bug 3 — missing L2 PairingActivity.** §7.M specifies a three-layer
Mating model: L1 `ReproduceAspiration` (OpenMinded, High), L2
`PairingActivity` (OpenMinded, Medium), L3 `MateWithGoal` (SingleMinded,
High). Code shipped L1 and L3; **L2 was missing**. Without L2, the only
path to a Partners bond was passive drift (~0.65 max romantic in 900s,
only if cats stay within range the full window). **Fix (Shape A — full
spec):** new `DispositionKind::Pairing` variant, new `Action::Pair`,
new `StepKind::Pair`, new `HasPairingCandidate` ZST marker, new
`pairing.rs` author system (mirrors the mating-eligibility author but
gates on Friends-bond + orientation-compat), new self-state DSE
`pairing_activity_dse` (single-axis Logistic on `mating_deficit`,
midpoint 0.3, OpenMinded), new target-taking DSE
`pairing_activity_target_dse` (3 axes: nearness/fondness/romantic,
Friends-bond candidate filter), new `resolve_pairing` step (adjacency
→ bumps `rel.romantic` at `pairing_romantic_rate`, emits
`Feature::CourtshipInteraction`), new `build_pairing_chain` returning
`[MoveTo(target), Pair]`, new `GoapActionKind::Pair` arm in
`goap.rs`'s dispatcher, new `Pairing` arms in `commitment.rs`
(OpenMinded strategy + desire-drift `still_goal` against
`pairing_satiation_threshold`), new constants
`pairing_proximity_threshold`, `pairing_romantic_rate`, `pair_duration`,
`pairing_satiation_threshold`. The wiring is load-bearing in two
non-obvious places that would otherwise leave the L2 disposition
**dead-wired**: (1) `score_actions` must call
`score_dse_by_id("pairing_activity")` and emit `Action::Pair` so
Pairing enters the softmax pool, (2) `evaluate_and_plan` /
`evaluate_dispositions` must snapshot `HasPairingCandidate::KEY` into
`MarkerSnapshot` so the EligibilityFilter resolves. Without those two
sites the DSE registers but never fires — caught mid-Bug-3 by a soak
that emitted zero `CourtshipInteraction` and zero `Pairing`
PlanCreated despite the registration appearing complete.

**Bug 3 verification.** Seed-42 release deep-soak
(`logs/tuned-42-027bug3/`) + Mocha focal trace
(`logs/tuned-42-027bug3-trace/`):

| metric | pre-Bug-3 (post-Bug-2 baseline) | post-Bug-3 |
|---|---|---|
| Pairing dispositions adopted | 0 (variant didn't exist) | **548** plans created across 2 cats (Mocha 303, Birch 245) |
| `Pair` chosen at L3 (Mocha focal trace) | n/a | **2,254** of ~36k L3 records |
| `Feature::CourtshipInteraction` | 1,174 (passive only) | **3,268** (passive + active) |
| `BondFormed` cumulative | 3 | 1 (single Friends pair Mocha↔Birch — within noise; substrate works) |
| `MatingOccurred` | 0 | 0 (cascade does not close in 900s on this seed — see below) |
| shadowfox_ambush_deaths | 4 | 5 |
| starvation_deaths | 0 | 1 (noise band per CLAUDE.md) |

L3-trace acceptance criterion ("Pairing winning at least once before
MateWithGoal fires") cleared by 2,254×. **`MatingOccurred > 0`** is
*not* hit on this seed: the Mocha↔Birch Friends bond formed at tick
1264300 (~67k ticks before sim end), Pairing then drove 3,268
CourtshipInteraction events between them, but with
`pairing_romantic_rate = 0.0075` and `partners_romantic_threshold = 0.5`
the cascade doesn't close into a Partners bond before the 900s window
expires. The substrate is functionally correct (verifiable in the
focal trace); the remaining bar is the L1→L2→L3 cascade *time-budget*
— explicitly out of scope for Bug 3 per the ticket's "Coefficient
tuning of `courtship_*` constants" deferral. Multi-seed sweep +
`pairing_romantic_rate` tuning is the documented follow-up balance
thread; ticket 014's mating-cadence target is now reachable in
principle, with the substrate gap closed.

**Tests.** 17 new across `pairing.rs`, `pairing_activity.rs`,
`pairing_activity_target.rs`, `pair.rs`, plus extended
`disposition.rs::tests`. Bug 1 added 1 test; Bug 2 added 4. 1320 / 1320
lib tests green.

**Files (Bug 3, the largest of the three).**
`src/components/markers.rs`, `src/components/disposition.rs`,
`src/components/task_chain.rs`, `src/components/goap_plan.rs`,
`src/ai/mod.rs`, `src/ai/dses/mod.rs`,
`src/ai/dses/pairing_activity.rs` (new),
`src/ai/dses/pairing_activity_target.rs` (new),
`src/ai/pairing.rs` (new), `src/ai/scoring.rs`, `src/ai/modifier.rs`,
`src/ai/commitment.rs`, `src/ai/planner/mod.rs`,
`src/ai/planner/actions.rs`, `src/ai/planner/goals.rs`,
`src/steps/disposition/mod.rs`, `src/steps/disposition/pair.rs` (new),
`src/systems/disposition.rs`, `src/systems/goap.rs`,
`src/systems/narrative.rs`, `src/plugins/simulation.rs`,
`src/resources/sim_constants.rs`, `examples/template_prompt.rs`,
`docs/wiki/*.md` (regenerated).
