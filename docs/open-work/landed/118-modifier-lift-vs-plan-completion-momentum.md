...
- 2026-05-06: Landed via design-space option 2 — per-modifier preemption flag. New ScoreModifier::preempts_in_flight method, defaulting to false; acute-class lurch modifiers 047 / 102 / 105 / 108 override to fire when their trigger scalar crosses the lurch threshold AND lift > 0 (inert-modifier guard prevents oscillation). New check_modifier_preemption system runs once per tick before evaluate_and_plan, iterates the modifier pipeline for each cat with GoapPlan (skipping Resting / Eating), on first true drops the plan + resets ticks_remaining + emits Feature::ModifierPreemption + L3PlanFailure focal-trace row + flips MomentumSummary.preempted. New scenario modifier_preempts_hunt activates lifts via constants override and asserts the path fires end-to-end. Seed-42 production soak: Feature::ModifierPreemption = 0 (substrate correctly dormant — lifts ship at 0.0 pending 119). Doctrine: docs/systems/distress-modifiers.md "Behavioral expression" + ai-substrate-refactor.md §7.5b. Unblocks 117 (social-warmth max shift) and 119 (CriticalHealth interrupt retirement).
