...
- ✓ landed ** 65** (done, —, score 0.86 (cross-cluster)) — §L2.10.7 SpatialConsideration roster sweep (cat self-state DSEs + fox dispositi…
- · **219** (ready, ai-substrate, score 0.85) — shared recent-ambush event marker
- ✓ landed ** 49** (done, —, score 0.85 (cross-cluster)) — §9.2 faction overlay markers
