...
itself doesn't exist; it lands with C4 (cluster C / ticket 128, the
HTN method composition substrate spun out of 007 §C4).
...
- The coordinator DSE itself (that's C4 in cluster C / ticket 128).
- Other §7.3 row finalizations.

## Approach

When 128 (C4 HTN method composition / strategist-coordinator track)
lands, wire this row in the same PR. Mechanically: extend
...
- · **129** (blocked, C, score 0.85 (cross-cluster)) — Care DSEs over perceivable intentions
- · **127** (blocked, C, score 0.84 (cross-cluster)) — Joint-intention substrate for two-cat practices
- ✓ landed ** 81** (done, planning-substrate, score 0.84 (cross-cluster)) — Coordination directive-failure demotion (colony-side stuck-loop guard)
...
- 2026-05-13: blocked-by repointed [007] → [128] as part of cluster-C
