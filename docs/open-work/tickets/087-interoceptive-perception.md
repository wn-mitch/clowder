...
- 2026-04-30: Implementation deviation. The plan called for a separate `Rest` DSE distinct from `Sleep`; on inspection that would require new DSE-id-to-action plumbing throughout the catalog (multiple sites in `scoring.rs` / `eval.rs` / `disposition.rs` look up DSEs by hardcoded id strings, and `populate_dse_registry` would need a wired step resolver). Sleep already produces the `Resting` disposition that `disposition.rs:263`'s critical-health interrupt special-cases, so the user's symptom-level ask ("cats at low health should flee or rest") is satisfied at the scoring layer by adopting `pain_level` as a fifth Sleep axis (bumps Sleep's score for wounded cats without restructuring the catalog). A standalone Rest DSE — distinct semantics from Sleep, gated on `forbid(HasThreatNearby)`, mapping to a dedicated step resolver — is now deferred to a follow-on under §L2.10 catalog enumeration. Documenting here so the deviation doesn't get rediscovered as a fresh idea.
- 2026-04-30: Implementation deviation #2. Flee's new `health_deficit` axis was initially Logistic (gating semantics) per the plan; soak-suite test `cautious_cat_flees_when_threatened` caught the regression (full-health cat under threat picks Cook over Flee because the Logistic crashes the CompensatedProduct). Switched to `Linear { slope=0.4, intercept=0.6 }` so the axis floors at 0.6 (full-health → bonus lift, not gate) and saturates at 1.0 (full deficit → 1.0). Wounded cats still flee harder than healthy cats under identical threat conditions; healthy cats still flee. Test added (`wounded_cat_scores_flee_above_healthy_cat_under_threat`) to lock the invariant.
- 2026-04-30: Soak observation. `just soak 42` against `logs/tuned-42-pre-087` baseline (commit e838bb7, `CriticalHealth` interrupts = 19,382 / Starvation = 0 / 3 ShadowFox deaths) produced unexpected colony state in the logged tail-window: 8 founder cats, 0 PlanInterrupted of any kind, action mix collapsed to `Eat 62% / Build 13% / PracticeMagic 7.7% / Herbcraft 7.2% / Coordinate 5% / Socialize 3.2% / Sleep 1.1% / Wander 0.3%` (zero Forage / Hunt / Cook / Groom / Mate / Patrol / Fight / Flee / Idle), Stores building reached capacity 50 but `current = 0` (never filled), 8 cats starved in evenly-spaced cascade ticks 1209999 → 1211999. Both runs identical wildlife / prey populations at tick 1.2M. `Eat` action recorded but `FoodEaten` Feature never fired — cats appear to be replanning through `eat_at_stores` without ever completing the resolver. Reads as a balancing / DSE→GOAP plan-execution issue rather than a strict sim regression: the perception substrate plus the Sleep/Flee axis additions shifted scoring landscape enough that cats commit to Eat/Build/Crafting plans the colony can't fulfill (no producer-side actions). User holds the investigation; not opening a follow-up ticket.
