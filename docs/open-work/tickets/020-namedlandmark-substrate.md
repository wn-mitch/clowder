...
- · **124** (ready, ai-substrate, score 0.84 (cross-cluster)) — `LandmarkAnchor::OwnTerritoryCenter` — third interoceptive self-anchor (territo…
- ✓ landed ** 98** (done, docs, score 0.83 (cross-cluster)) — Document the substrate-vs-search-state boundary in `docs/systems/ai-substrate-r…
- ✓ landed ** 52** (done, —, score 0.83) — §L2.10.7 plan-cost feedback substrate + cat target-taking roster
