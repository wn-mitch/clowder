...
- ✓ landed ** 14** (done, —, score 0.92 (cross-cluster)) — "§4.2 State marker trio — `InCombat` / `OnCorruptedTile` / `OnSpecialTerrain` a…
- ✓ landed ** 92** (done, —, score 0.89 (cross-cluster)) — Unify MarkerSnapshot ↔ PlannerState/StatePredicate feasibility languages
- ✓ landed **160** (done, ai-substrate, score 0.89) — Substrate stub catalogue + lint — fail the build on orphan markers
