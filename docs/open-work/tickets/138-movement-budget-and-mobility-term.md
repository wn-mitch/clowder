...
- ✓ landed **121** (done, substrate-over-override, score 0.87 (cross-cluster)) — Cats stand around for ~1500 ticks at game start until first kitchen lands
- · **  1** (in-progress, —, score 0.87 (cross-cluster)) — Explore dominance over targeted leisure
- ✓ landed **163** (done, ai-substrate, score 0.86 (cross-cluster)) — Migrate the 9 apply_*_bonus passes into §3.5.1 modifiers (full-batch)
