...
- ✓ landed ** 14** (done, —, score 0.86 (cross-cluster)) — "§4.2 State marker trio — `InCombat` / `OnCorruptedTile` / `OnSpecialTerrain` a…
- · **  1** (in-progress, —, score 0.86 (cross-cluster)) — Explore dominance over targeted leisure
- ✓ landed ** 73** (done, —, score 0.85 (cross-cluster)) — Wave 2 substrate hardening
