...
Load-bearing for `slot-inventory.md` (first producer of wearables), `ruin-clearings.md` (loot routes to crafting materials, not finished gear), and `naming.md` (Phase 3 and Phase 4 are two of the six NamedLandmark consumers). Crafting is the anchor of the items-crafting line; ship it first.
...
Drift toward random/decoupled stat-stick fields or RNG affix rolls is a thesis-breaking change. Identity/material-grounded effect-data composed by the aggregation layer is the sanctioned shape, not drift.
