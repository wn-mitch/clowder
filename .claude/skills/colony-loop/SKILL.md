...
### 2. Plan-confirm via AskUserQuestion (GATE 1)
...
AskUserQuestion options: **Approve** / **Edit cluster** / **Edit metric** /
...
- **swarm-safe** tickets → `just foreman-spawn <N>` where N = swarm-safe count in the
  ...
  to swarm-safe) as polecats exit. Polecats run `--model sonnet`.
...
- Launch a **fresh skeptic via Task** — a **different model than the sonnet workers**
  (use a strong reviewer/opus-tier agent), defaulting to REJECT, that ACTS (re-runs,
...
- **Generator ≠ evaluator.** Workers are sonnet polecats / session agents; the skeptic
