...
- Emit the starter prompt block verbatim — the user copy-pastes it into a new Claude session in the new workspace path.
...
- **Always print the starter prompt** for new sessions so the user can spawn a child Claude session manually — there's no automated child-spawning in Stage 1 (`/foreman` ships in Stage 2).
...
- Three-track table: CLAUDE.md addendum (Stage 1.7)
