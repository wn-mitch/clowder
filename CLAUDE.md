...
**Parallel-session orchestration (ticket 354 / [`docs/workflow/parallel-sessions.md`](docs/workflow/parallel-sessions.md)).** When running multiple Claude Code sessions in parallel, the operator surface is `/work` (daily driver) + `/retag` (one-shot corpus tagging). Sessions live at `~/clowder-sessions/<slug>/` as jj workspaces with their own working copy + target dir; `session/<slug>` is the bookmark namespace, owned by exactly one session and never moved by anyone else. `main` is read-only inside sessions — the only path to `main` is `just refinery --land <slug>`. Every active ticket carries an `orchestration:` frontmatter axis with three values (enforced by `scripts/check_orchestration_frontmatter.py` via `just check`):
...
- **`swarm-safe`**: docs / frontmatter migrations / mechanical refactors / atomic bugfixes with verified layer-walks. Polecat-eligible (Stage 2); refinery `--auto` is whitelisted in code to this track only.

Recipes (Claude's primitive surface; humans use `/work` / `/retag`): `[session]` lifecycle (`just session-new <slug>`, `session-list`, `session-done`); `[refinery]` (`just refinery [--land <slug>]`); `[retag]` (`just retag <id> --track <name>`, `retag-suggest`, `retag-audit`); `[block]` (`just block-list`, `block-info <id>`); `[ticket-query]` (`just ticket-info <id>`, `open-work-ready-filtered --track <name>`). Discoverability for me: `just --list | grep '\[<tag>\]'`. Polecats run swarm-safe only.
