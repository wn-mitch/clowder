...
///
/// **Ticket 450 / 451 dormancy note.** The 450 spec called for flipping
/// the data source from autonomic `Needs.hunger` to substrate-driven
/// `CurrentAction.action == BegForFood`. Ticket 451 lifted the
/// `Without<KittenDependency>` filter so kittens enter L2 scoring, but
/// the seed-42 verification soak revealed that adults preempt kitten
/// hunger via the direct-kitten-targeting Caretake path (FeedKitten
/// fires 4355× per soak), keeping kitten hunger above the BegForFood
/// threshold (0.5). The substrate-driven cry predicate would therefore
/// permanently silence the cry-map. The flip is parked behind a balance
/// follow-on that gives kittens a window to elect BegForFood before
/// adults preempt — see `Feature::KittenBegged`'s `expected_to_fire_per_soak`
/// comment in `system_activation.rs`.
///
