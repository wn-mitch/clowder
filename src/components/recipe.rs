...
/// `Ord` (502): the registry's keyed-lookup index is a `BTreeMap` on
/// this id (iteration itself runs over the registration-order `Vec` —
/// see `RecipeRegistry`'s type-level doc for why that order is
/// load-bearing).
