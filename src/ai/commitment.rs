...
//! (`src/ai/dse.rs::CommitmentStrategy`). Phase 6a wires up the consumer:
//! a drop-trigger gate that fires each tick **after** the Maslow-interrupt
//! pipeline and decides whether the cat's currently-held Intention survives.
//! If the gate says drop, the plan is cleared and the next tick's softmax
//! selection picks a replacement.
//!
//! # Module boundaries
//!
//! This module owns:
...
//!   upstream from ECS state. Kept as a plain struct so unit tests can
//!   script truth tables without a `World`.
//! - [`should_drop_intention`] — the pure-function gate. One match arm per
//!   `CommitmentStrategy` variant; no I/O, no ECS access. Test-first.
//! - [`strategy_for_disposition`] — the §7.3 per-`DispositionKind` strategy
//!   table as a single `match`. Authoritative mapping; DSE factories
//!   committing a strategy for their `Intention::emit` should mirror this
//!   table per the class they serve.
//! - [`proxies_for_plan`] — integration helper that reads `GoapPlan` +
//!   `Needs` + constants and builds the `BeliefProxies` struct. Kept in
//!   this module so the proxy recipe is auditable next to the gate.
//!
...
//!   Intention's goal predicate. For held `GoapPlan`s, the per-
//!   `DispositionKind` completion check in
//!   `resolve_goap_plans`'s post-exhaustion block (`disposition_complete`,
//!   `goap.rs:~1672`) is the canonical achievement signal. Our proxy
//!   mirrors the same per-class recipe.
...
//!      iteration; wired as always-true today so the gate doesn't drop
//!      prematurely on a noisy re-score.
//!   2. **Hard-fail channel (b)** — `GoapPlan::replan_count <
//!      GoapPlan::max_replans` (`goap_plan.rs:103`). When the planner
//!      cannot route a step chain after the capped retries, achievability
//!      is believed lost regardless of strategy.
...
//!   (curiosity-drift — left always-true until the balance thread picks
//!   a concrete threshold). The proxy-gate dispatch is the deliverable
//!   here; balance-thread work tunes the thresholds.
//!
//! # Ordering in the tick
//!
//! Per spec §7.2 + refactor-plan Phase 6a:
//!
//! ```text
//! check_anxiety_interrupts (Maslow, §7.5 — bypasses gate + bonus)
//!   → drop-trigger gate (§7.2, this module)
//!   → evaluate_and_plan  (§L2.10.6 softmax picks replacement if dropped)
//!   → resolve_goap_plans (existing step executor)
//! ```
//!
//! The spec's `src/systems/disposition.rs:93` reference is the legacy
//! path; the live integration site is `src/systems/goap.rs::check_anxiety_interrupts`
//! and the drop-trigger gate is registered immediately after it in the
//! `SimulationPlugin` schedule (and mirrored in `build_schedule`).
//!
//! # Non-goals (deferred per scope)
//!
//! - **§7.4 persistence bonus.** Tier table + `logistic(completion_fraction)`
//!   shape is committed in the spec; numeric magnitudes are a balance
//!   thread. Not wired here.
//! - **§7.5 Maslow interrupt pipeline.** Existing `check_anxiety_interrupts`
//!   *already* runs before this gate and bypasses it (event-driven
//!   preemption installs a `Blind`-committed replacement). No new code.
//! - **§7.6 monitoring cadence.** Per-tick polling + event-driven
//!   interrupts are already in place.
...
// ---------------------------------------------------------------------------
// Integration system — the §7.2 drop-trigger gate
// ---------------------------------------------------------------------------

/// Drop-trigger reconsideration gate (§7.2). Runs each tick **after**
/// `check_anxiety_interrupts` (which bypasses this gate entirely per
/// §7.5) and **before** `evaluate_and_plan` (so a dropped plan lets the
/// next softmax pass install a replacement).
///
/// For each cat holding a [`GoapPlan`]:
///
/// 1. Look up its `CommitmentStrategy` via the §7.3 table
///    ([`strategy_for_disposition`]).
/// 2. Compute [`BeliefProxies`] via [`proxies_for_plan`].
/// 3. If [`should_drop_intention`] returns `true`, remove the plan —
///    the cat re-enters scoring next tick.
///
/// This system does **not** fire `PlanNarrative::Abandoned` — that
/// message is reserved for `Abandoned`-due-to-plan-failure in
/// `resolve_goap_plans` and `check_anxiety_interrupts`. A §7.2 drop
/// is closer to a clean reconsideration (e.g. goal achieved), not a
/// failure. The existing `PlanEvent::Completed` narrative fires from
/// `resolve_goap_plans` when the trip counter closes out; this gate
/// only drops when that path hasn't fired *but* the proxies say to
/// reconsider regardless — typically the `SingleMinded` hard-fail
/// case (`replan_count >= max_replans`) or an `OpenMinded`
/// satiated-desire drop.
pub fn reconsider_held_intentions(
    mut query: Query<(Entity, &GoapPlan, &Needs), Without<crate::components::physical::Dead>>,
    constants: Res<crate::resources::sim_constants::SimConstants>,
    mut commands: Commands,
    mut activation: ResMut<crate::resources::system_activation::SystemActivation>,
) {
    let d = &constants.disposition;
    for (entity, plan, needs) in &mut query {
        ...
        if should_drop_intention(strategy, proxies) {
            // Phase 6a telemetry: record that the gate fired. The
            // `Feature::CommitmentDropTriggered` feature lives on the
            // system-activation register (added alongside this gate);
            // per-cat narrative emission is Phase 6b scope.
            activation.record(
                crate::resources::system_activation::Feature::CommitmentDropTriggered,
            );
...
mod tests {
    ...
    fn integration_resting_survives_planner_pressure_until_needs_sated() {
        ...
        // `achievement_believed` (three-need completion) drops the plan.
        let d = default_d();
