...
/// Spatial influence map of colony garden buildings.
///
/// §5.6.3 row #10 of `docs/systems/ai-substrate-refactor.md` — sight ×
/// colony. Re-stamped each tick from live `Structure` entities whose
/// `kind` is `Garden`. Each functional garden paints a linear-falloff
/// disc of radius `garden_location_sense_range` weighted by
/// effectiveness so a `SpatialConsideration` consumer reads a
/// continuous "near garden infrastructure" gradient.
///
/// Producer-only landing per ticket 006. Consumer cutover (Tend /
/// Harvest target ranking via `SpatialConsideration`) is owned by
/// ticket 052.
#[derive(Resource, Debug, Clone, serde::Serialize, serde::Deserialize)]
pub struct GardenLocationMap {
    pub marks: Vec<f32>,
    pub grid_w: usize,
    pub grid_h: usize,
    pub bucket_size: i32,
...
impl Default for GardenLocationMap {
    fn default() -> Self {
        Self::default_map()
    }
}
...
mod tests {
    ...
    fn empty_map_reads_zero() {
        let map = GardenLocationMap::new(20, 20, 5);
        assert_eq!(map.get(10, 10), 0.0);
    }

    #[test]
    fn stamp_paints_falloff_within_radius() {
        let mut map = GardenLocationMap::new(40, 40, 5);
